...
from config import LAST_UPDATE, COMPARISONS_MARGIN
...
class WW2UkraineComparisonServer:
    def __init__(self, input, output, session):
        ...
        self.color_map = {
            "WW2 US to UK": "#2ca02c",
            "WW2 US to USSR": "#98df8a",
            "WW2 British to USSR": "#d62728",
            "WW2 US to France": "#ff7f0e",
            "British to Ukraine": "#1f77b4",
            "US to Ukraine": "#aec7e8",
        }
        ...
        for legend_name in self.color_map.keys():
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    orientation="h",
                    name=legend_name,
                    marker_color=self.color_map[legend_name],
                    text=text_values,
                    textposition="auto",
                    customdata=customdata,
                    hovertemplate=(
                        "GDP Share: %{customdata[0]:.2f}%<br>" + "Amount: %{customdata[1]:.2f}€B<br>" + "Conflict: %{customdata[2]}" + "<extra></extra>"
                    ),
                )
            )
